#!/usr/bin/env python3
"""
SCRIBE Agent — Generate word-level captions using Whisper.
Formats for Shorts-style animated text display.
...
OUTPUT_DIR = BASE_DIR / "output" / "captions"
...
def process_audio(audio_dir: str) -> None:
    ...
    for audio_file in sorted(audio_path.glob("*.wav")):
        ...
        out_file = OUTPUT_DIR / f"{audio_file.stem}.srt"
        try:
...
if __name__ == "__main__":
    audio_dir = sys.argv[1] if len(sys.argv) > 1 else str(BASE_DIR / "output" / "audio")
    process_audio(audio_dir)
